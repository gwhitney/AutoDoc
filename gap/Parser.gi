...
InstallGlobalFunction( AutoDoc_Type_Of_Item,
  function( current_item, type, default_chapter_data )
    ...
    declaration_cases := rec(
        ...
        := function (item)
            local rk;
            rk := rec( item_type := "Func", arguments := "arg",
                       doc_stream_type := "global_functions",
                       has_filters := "No" );
            if IsBound( item!.initial_args ) then
                rk.arguments := item!.initial_args;
            fi;
            return rk;
        end
        #! @BeginAutoDocPlainText
  ...
  function( filename_list, package_name, follow_package, tree, default_chapter_data )
        ...
        if declare_position <> fail and position_parentesis <> fail then
            ...
            if filter_string <> false then
                ...
                if not IsBound( man_item!.arguments ) then
                    if IsInt( has_filters ) then
                        if IsBound( man_item!.initial_args ) then
                            man_item!.arguments := man_item!.initial_args;
                        ...
                        if IsBound( man_item!.initial_args ) then
                            man_item!.initial_args := SplitString( man_item!.initial_args, "," );
                            man_item!.arguments{[ 1.. Minimum( Length( man_item!.arguments ),
                                                               Length( man_item!.initial_args ) ) ]}
                                := man_item!.initial_args;
                        elif Length( man_item!.arguments ) = 1 then
                            man_item!.arguments[ 1 ] := "arg";
                        fi;
    ...
    command_function_record := rec(
        ...
        @Arguments := function()
            ...
            item := new_man_item();
            if IsBound( item!.initial_args ) then
              item!.arguments := Concatenation( item!.initial_args, ", ",
                                                current_command[ 2 ] );
            else
                item!.arguments := current_command[ 2 ];
            fi;
        end,
        #! @Subsection @Group
        ...
        #! @SubsectionTitle @GroupTitle <A>title</A>
        #! @Index @GroupTitle `@GroupTitle`
        #!   Sets the subsection heading for the current group to
        #!   <A>title</A>. In the absence of any `@GroupTitle` command, the
        #!   heading will be the name of the first entry in the group. See
        #!   <Ref Sect="Section_Groups" /> for more information.
        @GroupTitle := function()
            local man_item, group_name, chap_info, group_obj;
            man_item := new_man_item();
            if not HasGroupName( man_item ) then
                ErrorWithPos( "found @GroupTitle with no Group set" );
            fi;
            group_name := GroupName( man_item );
            chap_info := fail;
            if HasChapterInfo( man_item ) then
                chap_info := ChapterInfo( man_item );
            elif IsBound( man_item!.chapter_info ) then
                chap_info := man_item!.chapter_info;
            fi;
            if (chap_info = fail or chap_info = [ ]) then
                chap_info := scope_chapter_info;
            fi;
        ...
        #! @Subsection @GroupInitialArguments
        #! @SubsectionTitle @GroupInitialArguments <A>args</A>
        #! @Index @GroupInitialArguments `@GroupInitialArguments`
        #!    Sets the common initial arguments for the entries in the
        #!    current group to be <A>args</A>. The argument list for any
        #!    function subsequently added to the group without any args
        #!    specified will be <A>args</A>, and <A>args</A> will be prepended
        #!    to any arguments specified for a particular function. The
        #!    setting is cleared by an `@EndGroup` command. See
        #!    <Ref Sect="Section_Groups" /> for more information.
        @GroupInitialArguments := function()
            scope_initial_args := current_command[ 2 ];
            new_man_item()!.initial_args := current_command[ 2 ];
        end,
        #! @Subsection @SetLevel
